...
export const useVideoExporterEngine = ({
  ...
  const startVideoExport = async () => {
    ...
    if (ffScenesRef.current && ffScenesRef.current.length > 0 && Array.isArray(FULLFRAME_PACKS) && FULLFRAME_PACKS.length > 0) {

        const defaultPack = FULLFRAME_PACKS[0];

        for (const scene of ffScenesRef.current) {

            if (!scene.url) {

                const match = defaultPack.scenes.find((x: any) => x.role === scene.role && x.emotion === scene.emotion)

                            || defaultPack.scenes.find((x: any) => x.role === scene.role);

                if (match) {

                    scene.url = match.url;

                    scene.idbKey = match.idbKey;
