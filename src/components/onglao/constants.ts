// Generated constants and utility helpers for OngLao Platform

export const DEFAULT_SYSTEM_PROMPT = `Bản chỉ dẫn hành đạo — Ai lão
...
export const idb = {
    ...
    async setMany(items: Array<{ key: string; blob: Blob }>): Promise<void> {
        const db = await this.init();
        return new Promise((resolve, reject) => {
            const tx = db.transaction('assets', 'readwrite');
...
export let app: any = null, auth: any = null, db: any = null, appId: any = null;
...
export const LAO_GREETINGS_DB = {
  ...
  "boasting_ego": [
    ...
    "Gáy to thế này chắc chưa gặp thợ săn bao giờ. �? đời núi cao còn có núi cao hơn, bớt huênh hoang lại kẻo có ngày ngã đau không ai đỡ.",
...
export const processChromaKeyPixels = (ctx: any, width: any, height: any, settings: any) => {
...
export const drawContactShadow = (ctx: any, charX: any, charY: any, charW: any, charH: any, settings: any) => {
...
export const getHarmonizeFilter = (settings: any) => {
...
export const autoDetectBgColor = (mediaElement: any) => {
...
export const audioBufferToWav = (buffer: any) => {
...
export const getLaoHair = (style: string, color: string) => {
...
export const getLaoSvgString = (mouthOpen = 0, appearance: any = { robeColor: '#92400e', innerRobeColor: '#b45309', hairColor: '#e2e8f0', hairStyle: 'bald' }) => {
  ...
  return `<svg viewBox="0 0 300 400" xmlns="http://www.w3.org/2000/svg">
    ...
    <g transform="translate(60, 40) scale(0.9)">
      ...
      <ellipse cx="75" cy="92" rx="12" ry="${Math.max(8 - eyeSquint, 0.5)}" fill="#fff" />
      <ellipse cx="125" cy="92" rx="12" ry="${Math.max(8 - eyeSquint, 0.5)}" fill="#fff" />
...
export const getUserSvgString = (mouthOpen = 0, gender: any = undefined, age: any = undefined, bowState = 0, appearance: any = {}) => {
...
export const setDoc = (...args: any[]): Promise<any> => Promise.resolve();
export const getDoc = (...args: any[]): Promise<any> => Promise.resolve({ exists: () => false, data: () => null });
export const collection = (...args: any[]): any => null;
export const onSnapshot = (...args: any[]): any => (() => {});
export const writeBatch = (...args: any[]): any => ({
  set: () => {},
  update: () => {},
  delete: () => {},
  commit: () => Promise.resolve()
});
export const signInAnonymously = (...args: any[]): Promise<any> => Promise.resolve();
export const onAuthStateChanged = (auth: any, callback: any): any => {
  callback(null);
  return () => {};
};
