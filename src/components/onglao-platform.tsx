...
const OngLaoPlatform = ({ initialPoems = [], autoOpenVideoModal = false }: { initialPoems?: any[]; autoOpenVideoModal?: boolean }) => {
  ...
  const inputTextRef = useRef(''); // Ref để đọc giá trị hiện tại mà không gây re-render
  const inputElRef = useRef<HTMLInputElement | HTMLTextAreaElement | null>(null); // ref đến DOM element
  ...
  // mouthOpen cho audio TTS của platform (playVoice) — dùng ref để không gây re-render 60fps
  const mouthOpenRef = useRef(0);
  const [mouthOpen, _setMouthOpenState] = useState(0);
  const setMouthOpen = (v: number) => { mouthOpenRef.current = v; };
      ...
      for (let i = 0; i < cleanSentences.length; i++) {
        ...
        try {
          ...
          if (data && data.audioContent) {
             ...
             const rawMsg = e?.message || "Lỗi không xác định";
             let cleanMsg = rawMsg;
             try {
               const matchJson = rawMsg.match(/\{.*\}/);
               if (matchJson) {
                 const parsed = JSON.parse(matchJson[0]);
                 if (parsed.message) cleanMsg = parsed.message;
               }
             } catch {}
             showToastMsg(`⚠️ Lỗi tạo Pháp Âm (TTS): ${cleanMsg}`, 'error');
             hasError = true; break;
        }
      }
...
${movieInstruction}${knowledgeInstruction}${liveContext}`;
  ...
  const p = {
      ...
      diagnosticReport: videoExportState.diagnosticReport, downloadAllAudios: videoExportState.downloadAllAudios, downloadAudio: (url: string, filename: string) => { const a = document.createElement('a'); a.href = url; a.download = filename; a.click(); }, downloadCombinedAudio: videoExportState.downloadCombinedAudio, editSessionTitle, editingEmotionId: null, editingId, editingSessionId,
  ...
  }), [p, videoExportState, poemDbState, liveStreamingState, authState, sessions, showAiManager, voicePersonas, currentVoicePersonaId, generatedScriptText]);
