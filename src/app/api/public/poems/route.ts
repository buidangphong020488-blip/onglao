...
export async function GET() {
  ...
  try {
    ...
    console.warn("DB offline khi đọc Kệ Pháp, chuyển sang GiacNgo API fallback:", dbErr?.message || dbErr);
  }

  // Nếu CSDL rỗng hoặc lỗi kết nối DB, tự động fetch từ GiacNgo Documents API làm fallback
  if (poems.length === 0) {
    try {
